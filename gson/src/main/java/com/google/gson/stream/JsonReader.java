...
public class JsonReader implements Closeable {
  ...
  private int handleNonEmptyArray() throws IOException {
    ...
    switch (c) {
      ...
      case ';':
        checkLenient();
      case ',':
        return peekValue(JsonScope.NONEMPTY_ARRAY);
      default:
  ...
  private int handleDanglingName() throws IOException {
    stack[stackSize - 1] = JsonScope.NONEMPTY_OBJECT;

    int c = nextNonWhitespace(true);
    switch (c) {
      case ':':
        return peekValue(JsonScope.NONEMPTY_OBJECT);
      case '=':
        checkLenient();
  ...
  private int peekValue(int peekStack) throws IOException {
    ...
    switch (c) {
      case ']':
        ...
        // fall through for trailing commas
      case ';':
      case ',':
